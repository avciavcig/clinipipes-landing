...
export function applyIndexContact(html, S) {
  ...
  let out = String(html);
  const subject = encodeURIComponent('CliniPipes Destek');
  const body = encodeURIComponent('Merhaba,\n\n');
  const mailto = `mailto:${email}?subject=${subject}&body=${body}`;
  const tr = `Destek: ${email}`;
  const en = `Support: ${email}`;
  const onclick = 'onclick="return openContactMail(event,this)"';

  out = out.replace(/<a\s+href="#"\s+([^>]*\bdata-contact-mail\b[^>]*)>/gi, `<a href="${mailto}" ${onclick} $1>`);
  out = out.replace(/<a\s+href="mailto:[^"]*"\s+([^>]*\bdata-contact-mail\b[^>]*)>/gi, (match, attrs) => {
    const clean = attrs.replace(/\sonclick="[^"]*"/gi, '').trim();
    return `<a href="${mailto}" ${onclick} ${clean}>`;
  });

  out = out.replace(
    /(<a\s+href="mailto:[^"]*"\s+id="footerSupportLink"\s+data-contact-mail\s+)data-tr="[^"]*"\s+data-en="[^"]*"([^>]*>)[^<]*(<\/a>)/i,
    `<a href="${mailto}" id="footerSupportLink" data-contact-mail ${onclick} data-tr="${tr}" data-en="${en}">${tr}</a>`
  );
