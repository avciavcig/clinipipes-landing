...
function getProvisioningMode() {
  if (!isPortalIntegrationEnabled()) return 'manual';
  const secret = getWebhookSecret();
  const base = (process.env.CLINIC_PORTAL_URL || '').replace(/\/$/, '');
  if (!secret || !base) return 'manual';
  return 'auto';
}

module.exports = { provisionClinic: provisionClinic, postJson: postJson, isPortalIntegrationEnabled: isPortalIntegrationEnabled, getProvisioningMode: getProvisioningMode };
