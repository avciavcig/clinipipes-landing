...
function createAdminAuth(configPath, envPassword) {
  ...
  function verifyTotp(secret, code) {
    ...
    try {
      return !!verifySync({ secret: secret, token: String(code).replace(/\s/g, '') }).valid;
    } catch (e) {
      return false;
    }
  }
  ...
  function getTotpUri(secret) {
    return generateURI({ issuer: 'CliniPipes', label: 'admin', secret: secret });
  }
  ...
  function setSecurityHeaders(res, opts) {
    ...
    if (opts.admin) {
      res.setHeader('Content-Security-Policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://api.qrserver.com; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'");
    }
  }
