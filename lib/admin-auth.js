...
function createAdminAuth(configPath, envPassword) {
  ...
  function verifyTotp(secret, code) {
    ...
    const token = String(code || '').replace(/\D/g, '').slice(0, 8);
    if (!clean || !/^\d{6,8}$/.test(token)) return false;
    try {
  ...
  function generateTotpSecret() {
    return speakeasy.generateSecret({ length: 20, otpauth_url: false }).base32;
  }

  function getTotpUri(secret) {
    return speakeasy.otpauthURL({
      secret: normalizeTotpSecret(secret),
      label: 'CliniPipes:Admin',
      issuer: 'CliniPipes',
      encoding: 'base32',
      algorithm: 'sha1',
      digits: 6,
