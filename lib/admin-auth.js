...
function createAdminAuth(configPath, envPassword) {
  ...
  function applyEnvTotp(cfg) {
    ...
    if (!envSecret || cfg.totpDisabledExplicitly || cfg.twoFactorMethod === 'email') return cfg;
  ...
  function getTwoFactorMethod(cfg) {
    ...
    if (cfg.twoFactorMethod === 'email') return 'email';
    if (cfg.twoFactorMethod === 'totp' || cfg.totpSecret) return 'totp';
    return 'email';
  }

  function verifyActive2fa(cfg, sessToken, code) {
    var method = getTwoFactorMethod(cfg);
    var token = normalizeOtpCode(code);
    if (!/^\d{6}$/.test(token)) return false;
    if (method === 'email') {
      return verifyEmailOtp('active:' + sessToken, token);
    }
    return verifyTotp(cfg.totpSecret, token);
  }
  ...
  return {
    ...
    getTwoFactorMethod: getTwoFactorMethod,
    verifyActive2fa: verifyActive2fa,
